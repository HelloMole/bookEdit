...
export default function ImgPlugin(instance) {
    ...
    instance.on('UPDATED_IMG', (newOpt) => {
        opt.scale = newOpt.scale
        drawImg()
    })
    ...
    function drawImg () {
        ...
        // console.log('opt', scale)
        setCanvasStyle(imgCanvasEle, style)
        instance.el.appendChild(imgCanvasEle)
        let ctx = imgCanvasEle.getContext('2d')
        ctx.lineWidth = 5
        ctx.strokeRect(0, 0, actEditWidth, actEditHeight) 

        ctx.drawImage(image, (1 - scale) * 0.5 * actEditWidth, (1 - scale) * 0.5 * actEditHeight, actEditWidth * scale, actEditHeight * scale)
    }
