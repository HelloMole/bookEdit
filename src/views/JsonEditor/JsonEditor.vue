...
export default {
  ...
  render() {
    var createElement = h
    // console.log('调用了render界面', createElement)
    const nodes = [];
      ...
      if (Object.keys(fields).length) {
        Object.keys(fields).forEach(key => {
          ...
          if(field.component != null){
            ...
            if(field.hasOwnProperty('items') && field.type !== 'select'){
              element = components[`${ field.type }group`] || defaultGroup
              if(field.schemaType == 'array' && (field.type == 'textsplit' || field.type == 'textarea')){
                element = components['textarea']
                // fieldValue = fieldValue.join(',')
              }
            }else{
              element = components[field.type] || defaultInput
            }
          }
          // const element = field.component ? customComponent : 
          ...
          const input = {
            ...
            on: {
              input: event => {
                ...
                if(field.schemaType == "array" && field.type == 'textsplit'){
                ...
                console.log('on input', value, fieldName)
          ...
          if(input.props != null){
            if(input.props.type == 'textsplit'){
              input.props.type = 'textarea'
            }
          }else if(input.attrs != null){
            if(input.attrs.type == 'textsplit'){
          ...
          if (field.label && !option.disableWrappingLabel) {
            ...
            // console.log("labelOptions", components.label, labelOptions)
    ...
    const formOptions = this.elementOptions(components.form, {
      autocomplete: false, //this.autocomplete,
      novalidate: false //this.novalidate,
    });
    nodes.push(
      createElement(
  ...
  methods: {
    ...
    freshSchema(schema){
      ...
      // this.allFormNodes = []
      // this.createForm(this, this.fields);
    },
    /**
