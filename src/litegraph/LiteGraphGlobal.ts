...
export class LiteGraphGlobal {
  ...
  NODE_MODES_COLORS = ["#0B8236", "#FF8B00", "#333", "#2885D7", "#626"]
  ...
  registerNodeType(type: string, base_class: typeof LGraphNode): void {
    ...
    // console.log('base_class' , type, base_class.name)
    const classname = base_class.name

    const pos = type.lastIndexOf("/")
    base_class.category = type.substring(0, pos)

    base_class.title ||= classname  //设置显示的名称，如果没有，默认类名
    ...
    // console.log('registered_node_types',  this.Nodes)
  }
  ...
  getNodeTypesInCategory(category: string, filter?: string) {
    ...
    // console.log('getNodeTypesInCategory', category, filter, r)
    return r
  }
