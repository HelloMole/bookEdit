...
export interface LGraphConfig {
  ...
  align_to_grid?: any
  links_ontop?: any
  rootccNode?: any  //传入cocos的Node对象，用户其它组件访问
  assestTotal?: number  //当前需要加载的数量
  assestLoaded?: number //当前已经加载的数量
  mute?: boolean  //当前是否静音
  audioArray?: object //当前播放音频的节点id
  inPause: boolean  //当前是否暂停
}
...
export class LGraph implements LinkNetwork, Serialisable<SerialisableGraph> {
  ...
  *positionableItems(): Generator<LGraphNode | LGraphGroup | Reroute> {
  ...
  detachCanvas(graphcanvas: LGraphCanvas): void {
    ...
    for (const node of this._nodes) {
        if(node.onDetachCanvas != null){
          node.onDetachCanvas()
        }
    }
  }

  async startLoadComplete(){
    return new Promise((resolve, reject) => {
        ...
        // }
        // check()
        this.checkAssestLoadComplete = ()=>{
              if(this.config.assestTotal == this.config.assestLoaded){
                  console.log('所有资源都加载好了', this.config.assestTotal)
                  this.checkAssestLoadComplete = null
                  resolve(true)
              }
          }
    })
  ...
  async start(interval?: number): Promise<void> {
    ...
    if (
      ...
      on_frame()
      //在第一次onExecute之后执行
      this.sendEventToAllNodes("onStartAfter")
    } else {
  ...
  playSounds(arr, cb, yinggui){
    ...
    for(var i = 0; i < arr.length; i++){
        ...
        let robot = LiteGraph.createNode('output/sound') // new ['soundRobot'](config)
        this.add(robot)
  ...
  sendEventToAllNodes(
    ...
    // console.log('当前的执行顺序', this._nodes_in_order)
    if (!nodes) return
    ...
    for (const node of nodes) {
      if (
          ...
          node.sendEventToAllNodes(eventname, params);
          continue;
      }

      // @ts-expect-error deprecated
      if (!node[eventname]) continue  // || node.mode != mode
      if (params === undefined) {
        // @ts-expect-error deprecated
        node[eventname]()
  ...
  configure(
    ...
    if (this.id === zeroUuid) this.id = createUuidv4()

    // console.log('当前配置的graph ID', data.id, data.id === zeroUuid, this.id)
