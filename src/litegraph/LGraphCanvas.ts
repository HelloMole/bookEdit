...
import { zhString } from "./zhString.js"
import type {
  CanvasColour,
  ColorOption,
  ConnectingLink,
...
export class LGraphCanvas implements ConnectionColorContext {
    ...
    function inner_onMenuAdded(base_category: string, prev_menu?: ContextMenu<string>): void {
      ...
      for (const node of nodes) {
        ...
        const entry: AddNodeMenu = {
          value: node.type,     //唯一标记，不可以修改
          content: node.title,  //显示的标题，可以翻译
          has_submenu: false,
          callback: function (value, event, mouseEvent, contextMenu) {
            if (!canvas.graph) throw new NullGraphError()
      ...
      console.log('构建菜单选项', base_category, entries )
  ...
  onMenuNodeToSubgraph(value, options, e, menu, node) {
		...
		var subgraph_node = LiteGraph.createNode("middle/graph/subgraph");
  ...
  /**
   ...
   /**
    ...
    openSubgraph = function(graph) {
        ...
        //打开子蓝图之前记录一下当前显示的位置和缩放，关闭后便于恢复
        // console.log('当前显示Area', graph, this.visible_area, this.ds)
        if(this.upVisibleArea == null){
          this.upVisibleArea = {}
        }
        this.upVisibleArea[this.graph.id] = {
          offset: [this.ds.offset[0], this.ds.offset[1]],
          scale: this.ds.scale
        }
        // this.upVisibleArea = {
        ...
        if(this.upVisibleArea[graph.id] != null){
          this.ds.scale = this.upVisibleArea[graph.id].scale
          this.ds.offset = this.upVisibleArea[graph.id].offset
        }
    ...
    if (this.graph) {
      ...
      if (this.dragging_rectangle) {
        ...
        if (eDown && eMove) {
          ...
          const ratio = 1//Math.max(1, window.devicePixelRatio)
        ...
        if (subnode.inputs)
            for (var i = 0; i < subnode.inputs.length; ++i) {
                ...
                if (this.drawButton(20, y + 2, w - 20, h - 2)) {
                    var type = subnode.constructor.input_node_type || "middle/graph/input";
                    this.graph.beforeChange();
        ...
        if (subnode.outputs)
            for (var i = 0; i < subnode.outputs.length; ++i) {
                ...
                if (this.drawButton(canvas_w - w, y + 2, w - 20, h - 2)) {
                    var type = subnode.constructor.output_node_type || "middle/graph/output";
                    this.graph.beforeChange();
                    var newnode = LiteGraph.createNode(type);
                    if (newnode) {
                        subgraph.add(newnode);
  ...
  //Draws a button into the canvas overlay and computes if it was clicked using the immediate gui paradigm
    ...
    // console.log(text + '按钮是否点击',was_clicked, clicked, this.block_click)
		if(clicked)
			this.blockClick();
		return was_clicked;
	}
  ...
  drawNodeShape(
    ...
    if (render_title || title_mode == TitleMode.TRANSPARENT_TITLE) {
      ...
      // custom title render
      node.onDrawTitle?.(ctx, this.graph_mouse)
    }
  ...
  showLinkMenu(segment: LinkSegment, e: CanvasMouseEvent): boolean {
    ...
    function inner_clicked(this: LGraphCanvas, v: string, options: unknown, e: MouseEvent) {
      ...
      switch (v) {
      case "Add Node":
        console.log('LGraphCanvas.onMenuAdd', menu)
        LGraphCanvas.onMenuAdd(null, null, e, menu, (node) => {
          // console.log('显示菜单项目node', node)
  ...
  showSubgraphPropertiesDialog(node: LGraphNode){
		console.log("showing subgraph input properties dialog");
  ...
  showSubgraphPropertiesDialogRight = function (node) {
    console.log("showing subgraph output properties dialog");
    var that = this;
