...
supported callbacks:
    ...
    + onGetInputs: returns an array of possible inputs 返回可能的输入
    + onGetOutputs: returns an array of possible outputs  返回可能的输输出
...
export class LGraphNode implements Positionable, IPinnable, IColorable {
  ...
  //当graph关联到一个canvas时触发
  onAttachCanvas?(this: LGraphNode, graph: LGraph): void
  //当graph取消关联到一个canvas时触发
  onDetachCanvas?(this: LGraphNode, graph: LGraph): void
  onDrawCollapsed?(
    this: LGraphNode,
    ctx: CanvasRenderingContext2D,
    cavnas: LGraphCanvas,
  ...
  onDrawTitle?(this: LGraphNode, ctx: CanvasRenderingContext2D, graph_mouse: Point): void
  onDrawTitleText?(
    this: LGraphNode,
    ctx: CanvasRenderingContext2D,
    title_height: number,
  ...
  isAnyInputConnected(): boolean {
    if (!this.inputs) return false
    let hasLink = false
    for(var slot = 0; slot < this.inputs.length; slot++){
      const input = this.inputs[slot]
      if (input.link != null) {
        hasLink = true
        break
  ...
  changeMode(modeTo: number): boolean {
    if(modeTo == LGraphEventMode.ON_TRIGGER){
      this.addOnTriggerInput()
      this.addOnExecutedOutput()
    }else{
      this.removeOnTriggerInput()
      this.removeOnExecutedOutput()
    }
    // switch (modeTo) {
  ...
  triggerSlot(
    ...
    for (const id of links) {
      ...
      if (node.mode === LGraphEventMode.ON_TRIGGER) {
        ...
        //Action中文翻译是行动的意思 ， Execute是执行的意思，原作者在这里定义两种触发类型的意义是？
  ...
  expandToFitContent(): void {
    ...
    // console.log('expandToFitContent', newSize)
    this.setSize([
  ...
  computeSize(out?: Size): Size {
    ...
    // console.log('node computeSize', ctorSize)
  ...
  //绘制错误的边框
  drawErrorTips(ctx: CanvasRenderingContext2D){
      ctx.beginPath()
      if(this.renderingShape == RenderShape.BOX){
          ctx.rect(0, -LiteGraph.NODE_TITLE_HEIGHT, this.renderingSize[0], LiteGraph.NODE_TITLE_HEIGHT)
      }else{
          ctx.roundRect(0, -LiteGraph.NODE_TITLE_HEIGHT, this.renderingSize[0], LiteGraph.NODE_TITLE_HEIGHT, [8, 8, 8, 8])
      }
      ctx.lineWidth = 2
  ...
  //绘制用于展示资源的box
  drawAssestBox(ctx: CanvasRenderingContext2D, assestWidth, assestHeight){
      var rectWidth = 0
      var rectHeight = 0
      var padding = 5
      var offsetX = 0
      var offsetY = 0
      if(this.flags.collapsed){
          rectWidth = this.renderingSize[0]
          rectHeight = this.renderingSize[0]
          if(rectHeight > assestHeight + padding * 2){
              rectHeight = assestHeight + padding * 2
          }
          offsetY = -7
      }else{
          rectWidth = 130
          ...
          offsetX = this.renderingSize[0] * 0.5 - rectWidth * 0.5
          offsetY = -30 - rectHeight
      }
      if(assestWidth == null){
        assestWidth = rectWidth
      }
      if(assestHeight == null){
        assestHeight = rectHeight
      }

      var maxScale = Math.min((rectWidth - padding) / assestWidth,  (rectHeight - padding) / assestHeight)
      // console.log('maxScale',maxScale)
      var newWidth = assestWidth * maxScale
      var newHeight = assestHeight * maxScale
      
      //进一步压缩需要绘制的矩形高度
      if(newHeight < newWidth){
          rectHeight = newHeight + padding
          if(!this.flags.collapsed){
            offsetY = -30 - rectHeight
          }
      }
      ...
      var x = offsetX + (rectWidth - newWidth) * 0.5
      var y = offsetY + (rectHeight - newHeight) * 0.5 

      ctx.beginPath()
      
      if(this.flags.collapsed){
          ctx.shadowColor = LiteGraph.DEFAULT_SHADOW_COLOR
          ctx.roundRect(0, offsetY, this.renderingSize[0], rectHeight, [0, 0, 8, 8])
      }else{
          ctx.roundRect(this.renderingSize[0] * 0.5 - rectWidth * 0.5, offsetY, rectWidth, rectHeight, [8, 8, 0, 0])
      }
      ctx.fillStyle  = this.renderingBgColor
      ctx.fill()
      ctx.shadowColor = "transparent"
      //同时返回素材绘制的区域
      let rect = {x: x, y: y, width: newWidth, height: newHeight}
      // console.log('返回了rect', rect)
      return rect
  }
  ...
  #arrangeWidgets(widgetStartY: number): void {
    ...
    for (const w of this.widgets) {
      if (this.isWidgetVisible(w) == false){
        continue
      } 
      if (w.computeSize) {
    ...
    if (y > bodyHeight) {
      // console.log('y > bodyHeight', y ,bodyHeight)
